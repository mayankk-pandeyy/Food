...
const Signup = ({setShowCard , setShowLogin}) => {
      ...
      <div className='w-[30%] font-dm-sans px-8 py-5 fixed top-28 z-20 left-[40%] bg-[#F5CB58] rounded-2xl'>
        ...
        <form className='mt-8 flex flex-col gap-3'>
          ...
          <div className='flex flex-col'>
            ...
            <div className='flex items-center justify-between px-4 py-1 rounded-xl text-[#03081F] bg-white'>
              <input type={showConfirmPassword ? "text" : "password"} placeholder='Confirm Password' className='w-full outline-none'/>
              {
                showConfirmPassword ? (<AiFillUnlock className='cursor-pointer' onClick={showConfirmPasswordHandler}/>) : (<TbPasswordUser className='cursor-pointer' onClick={showConfirmPasswordHandler}/>)
              }
            </div>
          </div>
        ...
        <div className='flex items-center mt-6'>
          ...
          <button className='bg-[#028643] text-white rounded-2xl mx-auto' onClick={()=>(setShowLogin(true))}>
